...
- (void)applyParameters:(NSDictionary *)someParameters
              onRequest:(NSMutableURLRequest *)aRequest;
- (void)applyHeaders:(NSDictionary *)someHeaders
           onRequest:(NSMutableURLRequest *)aRequest;
@end
...
+ (void)performMethod:(NSString *)aMethod
  ...
  NXOAuth2Request *request =
      [[NXOAuth2Request alloc] initWithResource:aResource
                                         method:aMethod
                                     parameters:someParameters];
  request.account = anAccount;
  request.headerFields = someHeaders;
  [request performRequestWithSendingProgressHandler:progressHandler
                                    responseHandler:responseHandler];
}

#pragma mark Lifecycle

- (instancetype)initWithResource:(NSURL *)aResource
...
- (NSURLRequest *)signedURLRequest;
{
  NSMutableURLRequest *request =
      [NSMutableURLRequest requestWithURL:self.resource];
  ...
  if (self.account.oauthClient.userAgent &&
      ![request valueForHTTPHeaderField:@"User-Agent"]) {
    [request setValue:self.account.oauthClient.userAgent
        forHTTPHeaderField:@"User-Agent"];
  }

  if (self.account) {
    NSString *oauthAuthorizationHeader =
        [NSString stringWithFormat:@"%@ %@", self.account.accessToken.tokenType,
                                   self.account.accessToken.accessToken];
    [request setValue:oauthAuthorizationHeader
...
- (void)performRequestWithSendingProgressHandler:
            (NXOAuth2ConnectionSendingProgressHandler)progressHandler
                                 responseHandler:
                                     (NXOAuth2ConnectionResponseHandler)
                                         responseHandler;
{
  NSAssert(self.me == nil,
           @"This object an only perform one request at the same time.");
              ...
              onRequest:(NSMutableURLRequest *)aRequest;
{
  ...
  if (![@[ @"POST", @"PUT", @"PATCH" ]
          containsObject:[httpMethod uppercaseString]]) {
    aRequest.URL = [aRequest.URL nxoauth2_URLByAddingParameters:someParameters];
  } else {
    NSInputStream *postBodyStream =
        [[NXOAuth2PostBodyStream alloc] initWithParameters:parameters];
    ...
    NSString *contentLength = [NSString
        stringWithFormat:@"%llu",
                         [(NXOAuth2PostBodyStream *)postBodyStream length]];
           ...
           onRequest:(NSMutableURLRequest *)aRequest;
{
  for (id key in someHeaders) {
    ...
    NSAssert1([key isKindOfClass:[NSString class]],
              @"Header name should be a string. name=%@", key);
    NSAssert2([value isKindOfClass:[NSString class]],
              @"Header value should be a string. name=%@, value=%@", key,
              value);
